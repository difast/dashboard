...
app.post('/api/bot/sandbox/topup',      requireAuth, (req, res) => {
    const p = new URLSearchParams({ amount: req.query.amount || 100000 });
    if (req.query.account_id) p.set('account_id', req.query.account_id);
    botProxy(req, res, 'POST', `/api/portfolio/sandbox/topup?${p}`);
})
